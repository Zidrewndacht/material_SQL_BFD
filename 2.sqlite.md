...
**Conclusão Técnica:** A parametrização separa o **código** (plano de controle, a estrutura do SQL) dos **dados** (plano de dados, os valores fornecidos pelo usuário). Isso torna impossível que dados de entrada alterem a lógica da query.
...
(solução apresentada no arquivo exemplos/2-exercício.py)
